  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Redirect to OTP verification page with email. In dev email mode the
        // backend echoes the OTP (response.devOtp) so the flow works without
        // a deliverable mail provider — never present in production.
        const devOtpParam = response.devOtp
          ? `&code=${encodeURIComponent(response.devOtp)}`
          : '';
        router.replace(`/verify-otp?email=${encodeURIComponent(formData.email)}${devOtpParam}`);
      }
    } catch (error) {
