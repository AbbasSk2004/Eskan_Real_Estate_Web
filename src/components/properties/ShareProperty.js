...
const ShareProperty = ({ property, className = '' }) => {
  ...
  // SSR-safe URL: `window` does not exist during the server render of this
  // client component, so resolve the origin lazily and only on the client.
  // propertyUrl is never rendered into markup — it is only used by the
  // client-side share handlers (window.open, clipboard, Web Share API).
  const propertyUrl = `${typeof window !== 'undefined' ? window.location.origin : ''}/properties/${property.slug || property.id}`;
