...
import { LEBANESE_GOVERNORATES } from '../../constants/lebanonLocations';
...
function AddPropertyForm({ onSubmitSuccess }) {
  ...
  // Strict numeric sanitizer: empty/invalid values are omitted entirely,
  // never coerced to NaN and stringified into the payload.
  const appendNumericIfFinite = (formData, key, value) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add numeric fields only if they hold valid finite values
      appendNumericIfFinite(formData, 'bedrooms', cleanBedrooms);
      appendNumericIfFinite(formData, 'bathrooms', cleanBathrooms);
      appendNumericIfFinite(formData, 'livingrooms', cleanLivingrooms);
      appendNumericIfFinite(formData, 'parking_spaces', cleanParkingSpaces);
      appendNumericIfFinite(formData, 'year_built', cleanYearBuilt);
      appendNumericIfFinite(formData, 'floor', cleanFloor);
      appendNumericIfFinite(formData, 'garden_area', cleanGardenArea);
      ...
      Object.entries(extraFields).forEach(([key, value]) => {
        ...
        // Strict sanitization: only finite numbers reach the payload — no empty/
        // non-numeric/NaN values are ever serialized.
        appendNumericIfFinite(formData, snakeKey, value);
      });
  ...
  <div className="col-md-6" key={field.name}>
    <div className="form-floating">
                    ...
                    <div className="col-12">
                      <div className="form-check">
                        ...
                        <label className="form-check-label" htmlFor="termsConditions">
                          I agree to the <button type="button" className="btn btn-link p-0 d-inline" onClick={() => setShowTerms(true)}>Terms and Conditions</button> and <button type="button" className="btn btn-link p-0 d-inline" onClick={() => setShowPrivacy(true)}>Privacy Policy</button>
                        </label>
                      </div>
                    </div>
      ...
      {/* Terms Modal */}
      <Modal show={showTerms} onHide={() => setShowTerms(false)} size="lg" centered scrollable>
      ...
      {/* Privacy Modal */}
      <Modal show={showPrivacy} onHide={() => setShowPrivacy(false)} size="lg" centered scrollable>
        <Modal.Header closeButton>
        ...
        <Modal.Footer>
          <button type="button" className="btn btn-primary" onClick={() => setShowPrivacy(false)}>I Understand</button>
        </Modal.Footer>
      </Modal>
    </div>
  );
