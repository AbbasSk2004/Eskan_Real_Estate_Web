...
const Sidebar = ({ 
  ...
  const { currentUser, isAuthenticated, logout, isInitializing } = useAuth();
  const pathname = usePathname();
  const { startFetching, unreadCount } = useNotification();

  // Use realtime notifications hook (WebSocket-driven)
  const {
    notifications,
      ...
      <div className={sidebarClasses} style={{
        ...
        {isAuthenticated && currentUser && (
          <div className="sidebar-user p-4 bg-light border-bottom">
              ...
              <div className="d-flex align-items-center">
                {(() => {
                  const avatarSrc = currentUser.profile_photo || currentUser.profilePhoto || currentUser.profile?.profile_photo || currentUser.avatar || null;
                  return avatarSrc ? (
                    <img
                      src={getProfileImageUrl(avatarSrc)}
                      alt="Profile"
                ...
                <div>
                  <h6 className="mb-0">
                    {`${currentUser.firstname || currentUser.firstName || currentUser.profile?.firstname || currentUser.email?.split('@')[0] || 'User'} ${currentUser.lastname || currentUser.lastName || currentUser.profile?.lastname || ''}`.trim()}
                  </h6>
        ...
        <div className="sidebar-footer p-3 border-top mt-auto">
          {isInitializing ? (
            <div className="d-flex align-items-center justify-content-center py-2" aria-label="Loading authentication">
