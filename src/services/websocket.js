 ...
 *
 * Priority:
 *   1. NEXT_PUBLIC_WS_URL  – explicit ws:// / wss:// value
 *   2. NEXT_PUBLIC_API_URL – convert http(s) → ws(s) and strip /api suffix
...
class WebSocketService {
  ...
  connect() {
    ...
    // Don't try to connect without an authenticated session. The HttpOnly
    // cookie is sent automatically on the WebSocket handshake, so no token
    // needs to be passed.
    const isLoggedIn = typeof window !== 'undefined' && !!sessionStorage.getItem('user');
    if (!isLoggedIn) {
      console.log('WebSocket: No active session, skipping connection');
      return;
    }
