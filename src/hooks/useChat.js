...
export const useChat = () => {
  ...
  // Baseline conversation load — once per mount sequence. Live updates come
  // over WebSocket; the socket reconnect handler reconciles anything missed.
  useEffect(() => {
    if (isAuthenticated) {
  ...
  useEffect(() => {
    ...
    const unsubscribeNewMsg = websocketService.subscribe('new_message', (msg) => {
      ...
      // (skip own echoes — the sending tab already holds the optimistic copy,
      // and no duplicates are ever added via id-based dedupe)
      if (
      ...
      setConversations((prev) => {
        ...
        const updated = prev.map((conv) => {
          if (conv.id === msg.conversation_id) {
            ...
            const existingMsgs = Array.isArray(conv.messages) ? conv.messages : [];
            const newMsgs = existingMsgs.some((m) => m.id === msg.id) ? existingMsgs : [...existingMsgs, msg];
            return {
              ...conv,
              messages: newMsgs,
    ...
    // RECONNECT RECONCILE ----------------------------------------------
    // One REST sync every time the socket (re)connects — replaces the old
    // 10s poller as the catch-up mechanism for anything missed while offline.
    const unsubscribeConn = websocketService.subscribe('connection', ({ connected }) => {
      if (!connected) return;
      // Skip if the mount baseline is still pending — it covers the first
      // connect; only re-sync once the baseline completed (i.e. real reconnects).
  ...
  }, [isAuthenticated, fetchConversations, markMessagesAsRead, sortConversations, user, fetchMessages]);
